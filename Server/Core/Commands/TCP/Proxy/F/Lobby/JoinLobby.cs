public class JoinLobby()
{
    public static async Task Process(FPacket fPacket, Session session)
    {
        ...
        buffer = Writer.WriteBytes("BBHQLQ", LobbyCommandsClient.CONNECT, 0x00, 1, room.ID, 2, room.Host.EugenID);
        response = new(fPacket.channel, FClientOpcode.LobbyMessage, buffer);
        await ProxyReader.FinalizePacket(response.ToSend(),session);
        buffer = Writer.WriteBytes("BBHQLQ", LobbyCommandsClient.CONNECT, 0x00, 0, room.ID, 6, session.EugenID);
        response = new(fPacket.channel, FClientOpcode.LobbyMessage, buffer);
        await ProxyReader.FinalizePacket(response.ToSend(),room.Host);
        buffer = Writer.WriteBytes("BBHLLQ", SystemMessageType.ON_LOBBY_ENTERED, 0x00, 0, -1, 0, room.ID);
        ...
        buffer = Writer.WriteBytes("BBHLLQs", 0x66, 0x00, 0, BitConverter.ToInt32(byteip),-1905684631, room.ID, "Relay.1");
        response = new(fPacket.channel, FClientOpcode.SystemMessage_2, buffer);
        await ProxyReader.FinalizePacket(response.ToSend(), session);
        buffer = Writer.WriteBytes("BBHLLQ", LobbyCommandsClient.MESSAGE_HOST_CHANGED, 0x00, 14754, 2, -1, room.ID); // h - 0x68
        response = new(fPacket.channel, FClientOpcode.LobbyMessage, buffer);
        await ProxyReader.FinalizePacket(response.ToSend(), session);
        buffer = Writer.WriteBytes("BBHLLQ", LobbyCommandsClient.LOBBY_ENTER_FINISHED,  0x00, 14754, 6, -1, room.ID); // j - 0x6A
        response = new(fPacket.channel, FClientOpcode.LobbyMessage, buffer);
        await ProxyReader.FinalizePacket(response.ToSend(), session);
        buffer = Writer.WriteBytes("BBHLLQ", SystemMessageType.JOIN_LOBBY_FINISHED, 0x00, 0, 6, 1, room.ID); 
        response = new(fPacket.channel, FClientOpcode.SystemMessage, buffer);
        await ProxyReader.FinalizePacket(response.ToSend(),room.Host);
    }
}