        ...
        try
        {
            ...
            if (requestOptions.Method == "POST")
            {
                if (requestOptions.Headers.TryGetValue("content-length", out var lengthStr) && int.TryParse(lengthStr, out int length) && length > 0)
                {
                    ...
                    {
                        int read = await ssl.ReadAsync(bodyBuffer.AsMemory(currentBodyRead, length - currentBodyRead), cts);
                        if (read == 0) break;
            ...
            await HTTPSInputReader.ReadTheInputHTTPS(requestOptions, responceOptions);
