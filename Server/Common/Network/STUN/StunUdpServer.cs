...
namespace stungun.common.server
{
    ...
    public class StunUdpServer : IDisposable
    {
        ...
        public void Start(ushort port, CancellationToken cancellationToken = default)
        {
            ...
            ServerLoop = Task.Run(async () => await ProcessingLoop(UdpServer, Configuration, cancellationToken), cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            do
            {
                ...
                try
                {
                    ...
                    {
                        Log.Error($"Received malformed message: too short ({udpReceive.Buffer.Length} bytes)");
                        continue;
                    }
                    ...
                    switch (req.Header.Type)
                    {
                        ...
                        default:
                            Log.Error($"Unknown message type: 0x{(ushort)req.Header.Type:X4}");
            ...
            StunServerConfiguration config)
        {
            ...
            Log.Error($"Sending BindingResponse with {readOnlyAttributes.Count} attributes");
            resp.PrintDebug();
            ...
            string? reason = null)
        {
           ...
           Log.Error($"Sending BindingError {errorCode}: {errorAttr.ReasonPhrase}");
            resp.PrintDebug();
            ...
            IEnumerable<ushort> unknownTypes)
        {
            ...
            Log.Error($"Sending BindingError 420 with unknown attributes: {unknownAttr}");
            resp.PrintDebug();
